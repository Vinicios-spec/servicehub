...
if(!$descricao || strlen($descricao) < 10){
        header("location: contratar.php?erro=Descreva melhor o problema (minímo 10 caracteres).");
    exit();
}
...
if ($usuarioBanco->buscarPorEmail($email)==false){
    // se retornou falso é por que não tem usuário com este emails no banco
    // então gravamos!!!!!!!!!!
    $usuario = new Usuario();
